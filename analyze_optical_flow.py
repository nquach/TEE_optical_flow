...
def sysdia_frames_by_angle(ds, label, smooth_fraction=0.2, pad_len=20, recalculate=False, verbose=False,
												save_angle_plot=False, save_dir=None, show_plot=False):
	if not recalculate and ds.CARDIACCYCLE_CALCULATED:
		if verbose:
			print('Cardiac cycle info calculated already! skipping calculation by angle!')
		return #skip if not recalculating and systolic and diastolic frames were already calculated
	...
	smoother_ang = SpectralSmoother(smooth_fraction=smooth_fraction, pad_len=pad_len)
	...
	if save_angle_plot or show_plot:
		frame_list = range(ds.nframes)
		fig, ax = plt.subplots(nrows=1,ncols=1)
		ax.plot(frame_list, ang_mode_arr, filt_ang_arr)
		for start, stop in sys_frames:
			ax.axvspan(start, stop, facecolor='0.8', alpha=0.5)
		for start, stop in dia_frames:
			ax.axvspan(start, stop, facecolor='0.9', alpha=0.5)

		if save_dir != None and save_angle_plot:
			safe_makedir(save_dir)
			save_name = ds.filename + '_' + label + '_' + param + '_sysdia_angle_diagnostic_plot.png'
			save_path = os.path.join(save_dir, save_name)
			fig.savefig(save_path)
		else:
			if save_angle_plot:
				print('ERROR save_dir cant be None if save_area_plot flag is True!')
		if not show_plot:
			plt.close(fig)
...
def sysdia_frames_by_area(ds, label, smooth_fraction=0.3, pad_len=20, recalculate=False, verbose=False,
	...
	area_list = []
	for i in tqdm(range(ds.nframes)):
		mask = mask_arr[i,:,:,0]
	...
	filt_area_baseline = peakutils.baseline(filt_area_list) #calculate baseline correction
	filt_area_list = np.asarray(filt_area_list) - np.asarray(filt_area_baseline) #remove baseline
	peak_i = sorted(list(peakutils.peak.indexes(filt_area_list, thres=dia_thres, min_dist=5)))
	val_i = sorted(list(peakutils.peak.indexes(filt_area_list * -1, thres=sys_thres, min_dist=5)))
	...
	for i in range(len(val_i)-1):
		...
		if len(dia_frames) == 0:
			del_frame = np.argwhere(filt_area_list == max(filt_area_list[v1], filt_area_list[v2])) #frame number of the systolic valley that is higher than the other
			del_frame_list.append(val_i.index(del_frame)) #marked down the index of the systolic area valley frame that is a local minimum
	...
	if save_area_plot or show_plot:
		frame_list = range(ds.nframes)
		fig, ax = plt.subplots(nrows=1,ncols=1)
		ax.plot(frame_list, area_list, filt_area_list)
		for start, stop in sys_frames:
			ax.axvspan(frame_list[start], frame_list[stop], facecolor='0.8', alpha=0.5)
		for start, stop in dia_frames:
			ax.axvspan(frame_list[start], frame_list[stop], facecolor='0.9', alpha=0.5)

		if save_dir != None and save_area_plot:
			save_name = ds.filename + '_' + label + '_area_plot.png'
			save_path = os.path.join(save_dir, save_name)
			fig.savefig(save_path)
		else:
			if save_area_plot:
...
def sysdia_frames_by_RTime(ds, recalculate=False, verbose=False, rr_sys_ratio=0.333):
	...
	for i in range(ds.RWaveTimes.size-1):
		...
		rr_time = r2 - r1
		sys_end = r1 + rr_time * rr_sys_ratio
		sys_interval = [r1, sys_end]
		dia_interval = [sys_end, r2]
		sys_times.append(sys_interval)
		dia_times.append(dia_interval)
...
def sysdia_frames_by_ecg_lazy(ds, ecg_arr, sampling_rate=500, smooth_fraction=0.2, pad_len=20, recalculate=False, verbose=False,
	...
	_, rpeaks = nk.ecg_peaks(filt_ecg, sampling_rate=sampling_rate, method='khamis2016', correct_artifacts=True, show=False)
	...
	for i in range(len(r_i)-1):
		...
		sys_interval = [r1, sys_end]
		dia_interval = [sys_end, r2]
		sys_i.append(sys_interval)
		dia_i.append(dia_interval)
	...
	sys_frames = [[s[0], np.min([s[1] + sys_extension, ds.nframes-1])] for s in sys_frames] #add sys_extension frames to sys frames interval for better coverage
	if verbose:
		print('systole frames:', sys_frames)
		print('diastole frames:', dia_frames)

	if save_ecg_plot or show_plot:
		fig, ax = plt.subplots(nrows=1,ncols=1)
		ecg_times = np.arange(n_elem)*(1000/sampling_rate)
		ax.plot(ecg_times, filt_ecg)
		ax.set_xlabel('Time (msec)')
		ax.set_ylabel('Voltage (mV)')
		for start, stop in sys_i:
			ax.axvspan(ecg_times[start], ecg_times[stop], facecolor='0.8', alpha=0.5)
		for start, stop in dia_i:
			ax.axvspan(ecg_times[start], ecg_times[stop], facecolor='0.9', alpha=0.5)

		if save_dir != None and save_ecg_plot:
			safe_makedir(save_dir)
			save_name = ds.filename + '_' + label + '_' + param + '_sysdia_ecg_diagnostic_plot.png'
			save_path = os.path.join(save_dir, save_name)
			fig.savefig(save_path)
		else:
			if save_ecg_plot:
				print('ERROR save_dir cant be None if save_ecg_plot flag is True!')
		if not show_plot:
			plt.close(fig)
...
def sysdia_frames_by_ecg(ds, ecg_arr, sampling_rate=500, smooth_fraction=0.2, pad_len=20, recalculate=False, verbose=False,
	...
	t_i = []
	sys_i = []
	for index in range(len(r_i)-1):
		R_start = r_i[index].item()
		R_stop = r_i[index+1].item()
		delta_R_i = R_stop - R_start
		search_start = int(np.round(delta_R_i*rr_search_range[0] + R_start)) #rr_search_range is a fraction of RR interval to search for T wave
		search_end = int(np.round(delta_R_i*rr_search_range[1]) + R_start)
		segment = filt_ecg[search_start:search_end]
		candidate_i = peakutils.peak.indexes(segment, thres=t_peak_thres, min_dist=t_min_dist) + search_start
		if len(candidate_i) > 0:
			candidate_y = [filt_ecg[i] for i in candidate_i]
			index = np.argmax(candidate_y)
			t_i.append(int(candidate_i[index]))
			sys_i.append([R_start, int(candidate_i[index])])
...
def sysdia_frames_by_art(ds, art_arr, sampling_rate=125, smooth_fraction=0.2, pad_len=20, recalculate=False, verbose=False,
	...
	smoother_art = SpectralSmoother(smooth_fraction=smooth_fraction, pad_len=pad_len)
	smoother_art.smooth(art_arr)
	filt_art = np.squeeze(smoother_art.smooth_data[0])
	lows_i =	peakutils.peak.indexes(filt_art * -1, thres=low_peak_thres, min_dist=low_min_dist) - sys_upstroke_offset
	lows_i[lows_i < 0] = 0

	highs_i = []
	sys_i = []
	for index in range(len(lows_i) - 1):
		low_start = lows_i[index].item()
		low_stop = lows_i[index+1].item()
		segment = filt_art[low_start:low_stop]
		candidate_i = peakutils.peak.indexes(segment, thres=high_peak_thres, min_dist=high_min_dist) + low_start
		if len(candidate_i) > 0:
			candidate_y = [filt_art[i] for i in candidate_i]
			index = np.argmax(candidate_y)
			highs_i.append(int(candidate_i[index]))
			delta_lowhigh = int(candidate_i[index]) - low_start
			sys_stop = low_start + int(np.round(sys_upstroke_multiplier*delta_lowhigh))
			sys_i.append([low_start, sys_stop])

	n_elem = art_arr.size
	dia_i = []
	if sys_i[-1][1] < lows_i[-1]:
	...
	if verbose:
			print('systole frames:', sys_frames)
			print('diastole frames:', dia_frames)
	if save_art_plot or show_plot:
		fig, ax = plt.subplots(nrows=1,ncols=1)
		art_times = np.arange(n_elem)*(1000/sampling_rate)
		ax.plot(art_times, filt_art)
		ax.set_xlabel('Time (msec)')
		ax.set_ylabel('Pressure (mmHg)')
		for start, stop in sys_i:
			ax.axvspan(art_times[start], art_times[stop], facecolor='0.8', alpha=0.5)
		for start, stop in dia_i:
			ax.axvspan(art_times[start], art_times[stop], facecolor='0.9', alpha=0.5)

		if save_dir != None and save_art_plot:
			safe_makedir(save_dir)
			save_name = ds.filename + '_' + label + '_' + param + '_sysdia_art_diagnostic_plot.png'
			save_path = os.path.join(save_dir, save_name)
			fig.savefig(save_path)
		else:
			if save_art_plot:
				print('ERROR save_dir cant be None if save_art_plot flag is True!')
		if not show_plot:
			plt.close(fig)
...
def plot_radlong_heatmap(ds, param, save_dir, nbins=1000,
												 av_filter_flag=True, av_savgol_window=10, av_savgol_poly=4,
	...
	if show_sysdia:
		if not ds.CARDIACCYCLE_CALCULATED or recalculate:
			if cc_method == 'area':
				...
				sysdia_frames_by_angle(ds, cc_label, smooth_fraction=cc_smooth_fraction, pad_len=20, recalculate=recalculate, verbose=verbose,
	...
	if np.any(waveform_data == None):
		waveform_exists = False
	else:
		waveform_exists = True

	if waveform_exists and sampling_rate==None:
	...
	if ds.mode != 'otsu':
		if not ds.CARDIACCYCLE_CALCULATED or recalculate:
			if cc_method == 'area':
				...
				sysdia_frames_by_angle(ds, cc_label, smooth_fraction=cc_smooth_fraction, pad_len=cc_pad_len, recalculate=recalculate, verbose=verbose,
												save_angle_plot=save_cc_plot, save_dir=save_dir, show_plot=False)
			elif cc_method == 'metadata':
				sysdia_frames_by_RTime(ds, recalculate=recalculate, verbose=verbose, rr_sys_ratio=rr_sys_ratio)
			elif cc_method == 'ecg':
